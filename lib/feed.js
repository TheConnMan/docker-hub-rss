...
function buildFeedXml(repo, user, images) {
  ...
  images.forEach((image) => {
    ...
    const guid = String(image.id) + '-' + new Date(image.last_updated).getTime();
    out += '<item>';
...
function filterTags(tags, filters = {}) {
  ...
  const safeIncludeRegex = includeRegex ? new RegExp(includeRegex) : null;
  const safeExcludeRegex = excludeRegex ? new RegExp(excludeRegex) : null;
  return tags.filter(
    (t) =>
      (include.length === 0 || include.includes(t.name)) &&
      (exclude.length === 0 || !exclude.includes(t.name)) &&
      (!safeIncludeRegex || safeIncludeRegex.test(t.name)) &&
      (!safeExcludeRegex || !safeExcludeRegex.test(t.name)),
  );
}
...
async function generateFeed({
  ...
  if (!/^[a-zA-Z0-9_.-]+$/.test(username) || !/^[a-zA-Z0-9_.-]+$/.test(repository)) {
  ...
  // The shared token may be able to read private repos; anonymously those are
  // inaccessible (404). Never turn one into a public feed regardless of scope.
  if (repo.is_private) {
    throw new Error('Repository is private');
  }
  // The user lookup only supplies the optional gravatar image. Official image
  // routes (`/_/nginx`) call `/v2/users/_/`, which Docker Hub 404s; tolerate
  // that and degrade to no image rather than failing the whole feed.
  const user =
    (await dockerHubFetch(`${DOCKER_HUB_BASE}/users/${userSlug}/`, {
      jwt,
      allow404: true,
    })) || {};
